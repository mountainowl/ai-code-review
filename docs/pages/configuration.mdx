...
The tables below list fallback values. Omitting a key is equivalent to using its
documented default. Any string value can
...
Recommended sizing for one self-hosted poller handling **about 20 unique reviews per day**. Bubo
orchestrates reviews and shells out to your agent CLI — the model runs **remotely**
(bring-your-own-LLM), so the host does no inference. Local cost is the poller, a few
concurrent agent subprocesses, repo checkouts, and SQLite state.

| Resource | Minimum | Recommended | What drives it |
|---|---|---|---|
| CPU | 2 vCPU | 4 vCPU | Git checkout and JSON processing; most time is spent waiting on the LLM API. |
| RAM | 2 GB | 4 GB | the poller plus concurrent agent subprocesses (`max_merge_requests_per_poll`) |
| Disk | 10 GB free | 20 GB+ | repo worktrees (largest factor), the Python environment and Node.js runtime, SQLite, logs, reports |
| Network | outbound HTTPS | — | to your SCM and the LLM API; no inbound ports |

**Software**

- Python 3.14+ and Node.js (your agent CLI — Codex or Claude — is an npm package)
- `git`; plus `gh` for GitHub or `glab` for GitLab as needed
- Linux (x86-64 or arm64) or macOS; cron/systemd templates ship for Linux hosts

**Sizing notes**

- **Disk is repo-bound.** Each reviewed change is checked out to a worktree under
  [`[poller].state_dir`](#poller). Large monorepos dominate — budget a few times your
  largest repo.
- **RAM is concurrency-bound.** Each in-flight review runs an agent subprocess. At
  20/day reviews rarely overlap; on a small host keep
  [`max_merge_requests_per_poll`](#review) low (1–2) to cap the peak.
- **Scale past 20/day** by adding RAM and CPU for more concurrency, and disk for
  more or larger repos. The LLM API, not the host, is the throughput ceiling.

See [Features](/features) and the [Recipes](/recipes).
...
| Setting | Default | Purpose |
|---|---|---|
| `url` | `https://gitlab.com` | SCM host that the poller reads merge requests from. For self-hosted GitLab, keep `api_url` on the same host. |
...
| `min_confidence` | `0.85` | Minimum confidence required for a finding to post. Findings below this value are dropped. |
...
| `tone` | `"terse"` | Comment voice — `terse` / `collaborative` / `socratic` / `formal` / `casual`. Changes only how a finding reads. See [Review comment tone](#review-comment-tone). |
...
| `codex_sandbox` | `read-only` | Filesystem access for Codex review runs. See [Troubleshooting](/troubleshooting) for the bubblewrap/AppArmor caveat. |
...
> **One secret, and how it reaches the agent.** `llm_api_key` is the only secret
> here. By **default** the review agent authenticates with its **own login** —
> bubo runs the reviewer under a strict env allowlist, so the key is *never*
> placed in the agent's environment (the primary anti-exfiltration defense).
> Authenticate the CLI once with your key, e.g. Codex: `codex login --with-api-key`.
> The **one exception** is a custom `llm_base_url` (an OpenAI-compatible endpoint):
> it reads the key from the environment at request time, so in that mode — and only
> then — `LLM_API_KEY` is passed through to the agent and `bubo init` wires the
> Codex profile to the endpoint. `bubo` does not guess a provider-specific env-var
> name, so the old `llm_api_key_env` knob is deprecated and no longer required.
...
How `bubo-mcp` exposes itself. See [MCP server](/mcp) for client setup.
...
`[review].tone` chooses the *voice* of a posted finding. It controls wording
only — it does not change filtering, severity, or metrics. Bubo ships with
...
**Measure tone effectiveness.** The active tone is recorded per run on the
...
The same cookie-deletion finding rendered in each tone. Severity, evidence, and
confidence stay unchanged:
...
Models use inconsistent `category` labels (`logic` vs `code-logic`,
...
Unrecognized labels are normalized to `other` (a non-defect bucket), so an unknown category
...
self-reported 0.9 is not comparable across categories — it means very different
things for a `correctness` claim versus a `style` nit, and models are reliably
*overconfident* on low-value findings (the empirical run carried a
`Missing Semicolon` at 0.9 and an `Unsigned Comparison` at 1.0).
...
Suppression is **conservative** and does nothing on a fresh install. It activates
only after enough outcomes exist for that repository and category to clear both
`dispute_suppress_min_samples` and `dispute_suppress_threshold`, so enabling it
early can't silence findings off a thin signal.
...
### Scope: per-repo, and only on disputes — never on accepted findings

Two boundaries are deliberate and worth stating, because it's easy to expect
otherwise:

- **Per-repo.** The join above runs `where project = ?` — suppression is learned
  and applied *per repository*. A class one team dismisses on one repo has no
  effect anywhere else; each repo earns, and loses, its own suppressions.
- **Disputes, not acceptances.** The numerator is `disputed` + `false_positive`
  only. A finding a team *accepted as real but chose not to fix* — e.g. a
  committed credential left in place with an "approved / won't fix" reply — is a
  true positive, not a dispute, so it never moves a category toward suppression.

This is intentional. Suppression silences classes a team has shown are *noise*; it
must not generalize "we accepted this risk once" into "stop reporting this."
Teaching a security reviewer to stop flagging committed secrets because one repo
waved one through would be actively dangerous — so an accepted true
positive on one repo never suppresses the same finding here or anywhere else.
...
posted is first re-checked by N independent "is this real?" lenses; a finding
refuted by most lenses is dropped (recorded `refuted`) instead of posted. Each lens
...
### Default verification is correlated, not independent
...
- The per-change `verified` / `refuted` / `capped` counts are emitted as a
  `verification_summary` log line — **Bubo does not silently cap findings**.
...
`get_governance_report` MCP tool). See [operate.md](/operate),
"Governance report".
...
| `$BUBO_ROOT/deploy/templates/` | Cron + systemd templates with `{{ROOT}}` already substituted; ready for `sudo install` / `systemctl enable` (see [operate](/operate)) |
