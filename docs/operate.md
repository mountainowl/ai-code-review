...
## Troubleshooting (host & infra)

Operational gotchas that look like bubo problems but are really host/infra
configuration. Each entry is *symptom → cause → resolution*. These are
**DevOps/infra fixes** — bubo itself needs no change.

### A review returns "no findings" but the sandbox actually failed

**Symptom.** A review — most often an MCP-triggered `review_change` — completes
with **no findings, but it is not a genuine clean review**. The run transcript /
...
You may also notice a sudden **spike in `no_findings` results**. Codex exits 0,
so nothing flags it automatically — the agent simply never got to read the diff.

**Cause.** `[agents].codex_sandbox` (`read-only`, and also `workspace-write`)
makes Codex isolate the run with **bubblewrap**, which needs to create a user
namespace and configure loopback. The breakage is specific to how Codex was
installed:

- An **npm/global Codex install bundles its own `bwrap`** (`root:root`, not
  setuid, **no AppArmor profile**).
- On Ubuntu with `kernel.apparmor_restrict_unprivileged_userns = 1`, an
  unprofiled binary cannot create a user namespace, so `bwrap` fails at the uid
  map / loopback step and every shell/git tool the agent needs is rejected.
- The **poller usually survives** because its review diff is embedded in the
  prompt (no shell needed). The **MCP path fails** because it runs Codex nested
  and without a checked-out worktree, so the agent must reach for git tools that
  the sandbox blocks. (A sandbox also cannot initialize *inside* another
  sandbox.)

> **Why "Codex works when installed via apt".** A distro/apt Codex pulls in the
> system **bubblewrap** package, whose `bwrap` has an AppArmor profile the kernel
> allows — and Codex prefers a `bwrap` found on `PATH` over its bundled copy
> (its warning: *"could not find bubblewrap on PATH … will use the bundled
> bubblewrap in the meantime"*). So the *install method* decides which `bwrap`
> runs.

#### Recommended fix — allow the bundled `bwrap` via an AppArmor profile

For **AppArmor-based distros (Ubuntu, Debian, openSUSE)** this is the cleanest
fix: it grants the user-namespace capability to Codex's *own* bundled `bwrap`, so
the sandbox keeps working and Codex stays self-contained — no extra package, no
loosened host-wide policy.

Locate the bundled binary first (the path depends on where npm installed Codex):

```sh
# Resolve the codex-resources/bwrap that ships inside the npm package.
BWRAP=$(find "$(dirname "$(dirname "$(readlink -f "$(command -v codex)")")")" \
  -path '*codex-resources/bwrap' 2>/dev/null | head -1)
echo "$BWRAP"
```

**1. Check — confirm the userns restriction and reproduce the failure.**

```sh
# Confirm the AppArmor user-namespace restriction is on.
sysctl kernel.apparmor_restrict_unprivileged_userns
sysctl kernel.apparmor_restrict_unprivileged_unconfined

# Confirm AppArmor supports userns rules (need apparmor_parser ≥ 3.x / abi 4.0).
apparmor_parser --version
grep -R "userns" -n /etc/apparmor.d/abi /sys/kernel/security/apparmor/features 2>/dev/null | head

ls -l "$BWRAP"; "$BWRAP" --version

# Reproduce the failure.
"$BWRAP" --unshare-net --ro-bind / / --proc /proc --dev /dev /bin/echo ok
unshare -Urn bash -lc 'ip link set lo up && echo ok'

# Reproduce through Codex itself.
codex exec --profile bubo --skip-git-repo-check --ephemeral 'run: echo ok'
```
...
**2. Fix — install a profile for the bundled `bwrap`.** The profile attaches to
the binary by its absolute path; the profile *filename* itself is arbitrary, so a
short stable name is fine:
...
If a Codex upgrade changes the bundled `bwrap` path, re-run the locate step and
re-parse the profile (the attachment path inside it must match the new location).

> If `apparmor_parser --version` reports < 4.0, change `abi <abi/4.0>` to the
> abi your parser supports (e.g. `abi <abi/3.0>` on Ubuntu 22.04), or drop the
> `abi` line entirely.

**3. Verify.**

```sh
"$BWRAP" --ro-bind / / --proc /proc --dev /dev /bin/echo ok              # → ok
...
# Then bubo itself:
/usr/local/bubo/bin/env /usr/local/bubo/bin/bubo-poller --health
# And from a Codex MCP client: call the bubo `health` tool.
```

> **Still can't review an MR after this?** The bubblewrap/AppArmor problem is
> fixed, but an *ad-hoc dry-run* can fail for a separate reason: the nested Codex
> run has **no local checkout** or **no GitLab network access**. That is a
> different issue from this section.

#### Other distributions

- **RHEL / Fedora / Rocky / Alma** use **SELinux**, not AppArmor — the
  `kernel.apparmor_restrict_unprivileged_userns` sysctl does not exist and the
  profile above does not apply. User namespaces are usually permitted there; if
  they are not, check `sysctl user.max_user_namespaces` (must be `> 0`) rather
  than AppArmor.
- **Arch and other non-AppArmor, non-SELinux hosts** generally allow
  unprivileged userns out of the box; if `bwrap` still fails, confirm
  `kernel.unprivileged_userns_clone = 1`.

#### Alternatives (if you don't want to add a profile)

- **Install system bubblewrap** — `sudo apt install bubblewrap` (Debian/Ubuntu).
  Codex prefers a `bwrap` on `PATH` over its bundled copy, and the distro package
  ships an allowed profile. *Confirm:* `command -v bwrap` → `/usr/bin/bwrap`.
- **Drop Codex's sandbox on a dedicated, isolated host** — set
  `codex_sandbox = "danger-full-access"` in `[agents]`. Acceptable when the VM is
  single-purpose and credential-stripped. **Not `workspace-write`** — it is also
  bubblewrap-based and fails identically.
- **Avoid** disabling `kernel.apparmor_restrict_unprivileged_userns` host-wide
  unless the host is fully dedicated and isolated.

**Make it visible.** Because this hides as "no findings," ship the poller's
stdout logs to your observability stack and alert on an anomalous no-findings
rate (see [telemetry.md](telemetry.md)). To check a single run by hand, grep its
