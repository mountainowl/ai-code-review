...
from llm_reviewer.telemetry import TelemetryConfig, telemetry_config_from_dict
...
class ReviewConfig:
    ...
    provider:
        ...
        Selects which :mod:`llm_reviewer.provider` implementation the
...
def load_review_config(
    ...
    :mod:`llm_reviewer.poller`; passing it lets configuration warnings
    ...
    The ``LLM_REVIEWER_PROVIDER`` environment variable overrides
    ``[scm].provider`` from the file. This is how the ``gh-review-poller``
    ...
    override = os.environ.get("LLM_REVIEWER_PROVIDER")
    if override:
