...
* :mod:`llm_reviewer.db` — SQLite schema and all writers.
* :mod:`llm_reviewer.mcp` — JSON-RPC client for the GitLab MCP server.
* :mod:`llm_reviewer.gitlab` — REST client.
* :mod:`llm_reviewer.findings` — finding extraction, policy filter,
  diff-position mapping.
* :mod:`llm_reviewer.subproc` — bounded subprocess execution with
  process-group cleanup.
* :mod:`llm_reviewer.secrets` — credential redaction.
* :mod:`llm_reviewer.signals` — cooperative SIGTERM/SIGINT shutdown.
* :mod:`llm_reviewer.events` — structured JSON-line logging.
...
from llm_reviewer.db import record as _db_record
from llm_reviewer.db import record_finding as _db_record_finding
from llm_reviewer.events import log, now
from llm_reviewer.findings import (
    extract_findings,
...
from llm_reviewer.mcp import call_tool as mcp_call_tool
from llm_reviewer.paths import CONFIG, ROOT
from llm_reviewer.prompt import render_meta_prompt as _render_meta_prompt
from llm_reviewer.prompt import write_rendered_meta_prompt as write_rendered_prompt_file
from llm_reviewer.review_config import ReviewConfig, load_review_config, review_config_from_dict
from llm_reviewer.scm import ScmProvider, get_provider
from llm_reviewer.secrets import redact_secrets
from llm_reviewer.signals import (
    install_signal_handlers as _install_signal_handlers,
)
from llm_reviewer.signals import (
    shutdown_requested as _shutdown_requested,
)
from llm_reviewer.statuses import FindingStatus, ReviewMode, ReviewStatus
from llm_reviewer.subproc import kill_process_group
from llm_reviewer.subproc import run_bounded as run
...
def reviewer_env(source: Mapping[str, str], prompt: Path, max_findings: int) -> dict[str, str]:
    ...
    env["LLM_CODE_REVIEW_ROOT"] = str(ROOT)
    env["LLM_CODE_REVIEW_PROMPT"] = str(prompt)
    env["LLM_REVIEW_MAX_FINDINGS"] = str(
        positive_int(max_findings, "max_findings_per_merge_request")
    )
    env["LLM_REVIEWER_SKIP_AGENT_CONFIG_ENV"] = "1"
    return env
...
def write_rendered_meta_prompt(cfg: ReviewConfig) -> Path:
    ...
    ``LLM_CODE_REVIEW_PROMPT_SOURCE`` lets tests point at a different
    source file without touching the install directory.
    """
    source = Path(
        os.environ.get("LLM_CODE_REVIEW_PROMPT_SOURCE", paths.ROOT / "prompts" / "00-meta.md")
    )
...
def fork_worker(job: Path) -> int:
    ...
    command = (
        shlex.split(configured) if configured else [sys.executable, "-m", "llm_reviewer.poller"]
    )
    try:
...
def poll() -> int:
    ...
    :func:`llm_reviewer.scm.get_provider` and drives it. Returns the number
...
def check_health() -> int:
    ...
    can pipe ``mr-review-poller --health`` straight into a monitor.
    """
    try:
        init_db()
        cfg = read_config()
    ...
    from llm_reviewer.db import status_age_seconds as _status_age_seconds
...
def main() -> int:
    """CLI entry point for ``mr-review-poller``.
