...
from llm_reviewer.events import now
from llm_reviewer.hash_utils import stable_digest, stable_hash
from llm_reviewer.statuses import FindingStatus, ReviewMode, ReviewStatus
from llm_reviewer.telemetry import TokenUsage
...
def review_run_id(project: str, iid: int, sha: str) -> str:
    ...
    of the tuple — see :func:`llm_reviewer.hash_utils.stable_hash`.
    """
...
def count_inflight_workers() -> int:
    ...
    See :func:`llm_reviewer.poller.poll` for how the result is used as a
...
def latest_reviewed_row() -> tuple[str, str] | None:
    ...
    Used by :func:`llm_reviewer.poller.check_health`. ``None`` when the
...
def record_finding(
    *,
    ...
    :func:`llm_reviewer.findings.finding_body` so this module does not
