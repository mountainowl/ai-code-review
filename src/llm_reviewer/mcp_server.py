"""MCP server exposing llm-reviewer's review state and trigger.
...
* **Metrics** — ``health``, ``list_recent_reviews``, ``get_review``,
  ...
  against the SQLite state :mod:`llm_reviewer.db` owns.
* **Review** — ``review_change`` shells out to ``code-review-codex`` for
...
* ``LLM_REVIEWER_MCP_TRANSPORT=stdio`` (default) — Codex spawns the
  process per session; no auth (filesystem-scoped).
* ``LLM_REVIEWER_MCP_TRANSPORT=http`` — long-lived HTTP+SSE server bound
  to ``LLM_REVIEWER_MCP_HOST:LLM_REVIEWER_MCP_PORT``. Every request must
  carry ``Authorization: Bearer <LLM_REVIEWER_MCP_BEARER_TOKEN>``; missing
  or mismatched returns ``401``. No TLS — put behind a reverse proxy.

Launch: ``mcp-llm-reviewer`` console script (also wrapped by
...
from llm_reviewer import db
from llm_reviewer.codex_runner import configured_timeout_seconds
...
mcp: FastMCP = FastMCP("llm-reviewer")
...
def health() -> dict[str, Any]:
    ...
    Mirrors ``mr-review-poller --health`` but returns structured data
...
def review_change(
    ...
    Args:
        ...
        timeout_seconds: Wall-clock budget for the underlying
            ``code-review-codex`` subprocess. Defaults to
    ...
    timeout = (
        int(timeout_seconds) if timeout_seconds is not None else configured_timeout_seconds()
    )

    task = _build_review_task(provider, project, number)
    launcher = ROOT / "bin" / "code-review-codex"
...
def _http_settings() -> tuple[str, int, str]:
    ...
    starting an unauthenticated server when ``LLM_REVIEWER_MCP_BEARER_TOKEN``
    is empty — there is no safe default here.
    """
    host = os.environ.get("LLM_REVIEWER_MCP_HOST", "127.0.0.1").strip() or "127.0.0.1"
    port_raw = os.environ.get("LLM_REVIEWER_MCP_PORT", "8765").strip() or "8765"
    try:
        port = int(port_raw)
    except ValueError as exc:
        raise SystemExit(f"LLM_REVIEWER_MCP_PORT must be an integer, got {port_raw!r}") from exc
    token = os.environ.get("LLM_REVIEWER_MCP_BEARER_TOKEN", "").strip()
    if not token:
        raise SystemExit(
            "LLM_REVIEWER_MCP_BEARER_TOKEN must be set when "
            "LLM_REVIEWER_MCP_TRANSPORT=http (the static bearer token "
...
def main() -> None:
    """Entry point for the ``mcp-llm-reviewer`` console script.

    Reads ``LLM_REVIEWER_MCP_TRANSPORT`` (default ``"stdio"``) and dispatches:
    ...
    * ``"http"`` — binds an HTTP+SSE server; blocks until interrupted.
    ...
    """
    transport = os.environ.get("LLM_REVIEWER_MCP_TRANSPORT", "stdio").strip().lower()
    if transport == "stdio":
        mcp.run()
    elif transport == "http":
        _run_http()
        ...
        raise SystemExit(
            f"LLM_REVIEWER_MCP_TRANSPORT must be 'stdio' or 'http', got {transport!r}"
        )
