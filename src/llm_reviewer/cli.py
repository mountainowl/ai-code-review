"""Top-level ``llm-reviewer`` CLI — install, doctor, and asset placement.
...
* ``llm-reviewer init`` — write the deployable assets the runtime needs
  but Python packaging cannot place itself: ``~/.codex/config.toml``
  (with the load-bearing ``[profiles.llm-reviewer]`` block),
  ``~/.claude/settings.json``, and the per-host workspace under
  ``$LLM_CODE_REVIEW_ROOT`` (config seed, var/ tree, SQLite schema,
...
* ``llm-reviewer doctor`` — non-mutating health check. Returns non-zero
  on any verification failure (missing ``~/.codex/config.toml``,
  ``[profiles.llm-reviewer]`` block absent, missing ``env.toml``,
...
``llm_reviewer/_assets/`` inside the wheel, so it works the same whether
...
from llm_reviewer.db import init_db
...
_ASSETS_PACKAGE = "llm_reviewer._assets"

# Editable-install fallback. Hatchling's force-include places the deploy
# assets at llm_reviewer/_assets/ inside the WHEEL, but an editable install
...
# src/llm_reviewer/_assets/ this fallback becomes a no-op (and can be
...
_EDITABLE_FALLBACKS: dict[tuple[str, ...], str] = {
    ...
    ("llm-reviewer.cron",): "deploy/templates/llm-reviewer.cron",
    ("llm-reviewer.service",): "deploy/templates/llm-reviewer.service",
    ("llm-reviewer.timer",): "deploy/templates/llm-reviewer.timer",
...
class Action:
    """A single filesystem mutation produced by ``llm-reviewer init``.
...
def _plan_workspace(root: Path) -> list[Action]:
    """Directories the runtime expects under ``$LLM_CODE_REVIEW_ROOT``."""
    return [
...
def _retarget_paths(root: Path) -> None:
    ...
    from ``LLM_CODE_REVIEW_ROOT``. When ``--root`` overrides that in a
    single ``llm-reviewer init`` invocation, the module-level constants
...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    os.environ["LLM_CODE_REVIEW_ROOT"] = str(root)
    ...
    if args.dry_run:
        print(f"# llm-reviewer init --dry-run (root={root})")
    ...
    print(f"llm-reviewer initialized under {root}", file=sys.stderr)
    if not args.no_agent_config:
        print(
            "next steps: edit config/env.toml (tokens), then run "
            "`llm-reviewer doctor` to verify the install",
            file=sys.stderr,
        )
    return 0
...
def _check_env_toml(root: Path) -> Check:
    ...
    return Check(
        ...
        detail=f"{target} — run `llm-reviewer init` if missing",
    )
...
def _check_codex_profile(home: Path | None = None) -> Check:
    ...
    if not target.is_file():
        return Check(
            ...
            passed=False,
            detail=f"{target} missing — run `llm-reviewer init` (with agent config)",
        )
    body = target.read_text()
    has_profile = "[profiles.llm-reviewer]" in body
    return Check(
        label="codex: [profiles.llm-reviewer] block present",
        passed=has_profile,
        detail=(
            "missing — codex --profile llm-reviewer will abort; "
            "regenerate with `llm-reviewer init --force`"
            if not has_profile
            else f"{target}"
        ),
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="llm-reviewer",
        description="Install and verify the llm-reviewer runtime on this host.",
    )
    ...
    init_p.add_argument(
        ...
        type=Path,
        default=None,
        help="install root (default: $LLM_CODE_REVIEW_ROOT or ~/.local/share/llm-reviewer)",
    )
    init_p.add_argument(
        "--dry-run",
        action="store_true",
